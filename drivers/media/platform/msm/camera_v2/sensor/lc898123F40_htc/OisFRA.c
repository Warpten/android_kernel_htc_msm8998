...
#define		RAM_XIN1	HALL_FRA_XHOUTB	/* "B" meaning Before */
#define		RAM_XIN2	HALL_FRA_XHOUTA	/* "A" meaning After  */

#define		RAM_YSIN	HALL_FRA_YSININ
#define		RAM_YIN1	HALL_FRA_YHOUTB	/* "B" meaning Before */
#define		RAM_YIN2	HALL_FRA_YHOUTA	/* "A" meaning After  */

#define		RAM_ZSIN	CLAF_RAMA_AFSINE
#define		RAM_ZIN1	CLAF_RAMA_AFDEV
#define		RAM_ZIN2	CLAF_DELAY_AFDZ0

//#define		LPF_ENA		// Enable LPF filter
							// Disable LPF filter Enable BPF
...
//**************************
void	FRA_Avg( void ) ;
void	FRA_Calc( StFRAMes_t *, UINT_8 ) ;
UINT_32	Freq_Convert( float ) ;
...
// History			: First edition
//********************************************************************************
void	FRA_Calc( StFRAMes_t *PtFRAMes, UINT_8 UcMesCnt )
{
	float	SfTempGain, SfTempPhase ;
...
// History			: First edition
//********************************************************************************
void	MesStart_FRA_Single( UINT_8	UcSingleSweepSel, UINT_8	UcDirSel )
{
	UnllnVal		StMeasValueA, StMeasValueB ;
	INT_32			SlMeasureParameterA, SlMeasureParameterB ;
	float			SfTmp ;
	UINT_8	i ;
	UINT_32	UlSampleNum, UlReadVal ;
	...
	if( UcDirSel == X_DIR ) {
		SetTransDataAdr( SinWave_OutAddr	,	(UINT_32)RAM_XSIN ) ;								// Set Sine Wave Input RAM
	}else if( UcDirSel == Y_DIR ){
		SetTransDataAdr( SinWave_OutAddr	,	(UINT_32)RAM_YSIN ) ;								// Set Sine Wave Input RAM
#ifdef	SEL_CLOSED_AF
	}else{
		SetTransDataAdr( SinWave_OutAddr	,	(UINT_32)RAM_ZSIN ) ;								// Set Sine Wave Input RAM
#endif
	}
	...
	UlReadVal	= ( UINT_32 )( ( float )0x7FFFFFFF * ( FS_FREQ / ( FS_FREQ + M_PI * StFRAParam.StHostCom.SfFrqCom.SfFltVal ) ) ) ;
	...
	UlReadVal	= ( UINT_32 )( ( float )0x7FFFFFFF * ( StFRAParam.StHostCom.SfFrqCom.SfFltVal / (StFRAParam.StHostCom.SfFrqCom.SfFltVal + FS_FREQ /  M_PI ) ) ) ;
	...
	for( i = 0 ; i < StFRAParam.StHostCom.UcAvgCycl ; i++ ) {
		...
		RamWrite32A( StMeasFunc_PMC_UcCrossDetectA, 0x00000000 ) ;									// Phase detect flag clear
		...
		UlSampleNum	= ( UINT_32 )( FS_FREQ / StFRAParam.StHostCom.SfFrqCom.SfFltVal ) + 1 ;
...
void	MesStart_FRA_Continue( void )
{
	...
	RamRead32A( StMeasFunc_PMC_UcPhaseMesMode, &UlReadVal ) ;
	UlReadVal	|= 0x00000400 ;																		// SweepMode = 0x04
	RamWrite32A( StMeasFunc_PMC_UcPhaseMesMode, UlReadVal ) ;										// change measure filter coefficient


	for( i = 0 ; i < StFRAParam.StHostCom.UcAvgCycl ; i++ ) {
		RamRead32A( StMeasFunc_PMC_UcPhaseMesMode, &UlReadVal ) ;
		UlReadVal	|= 0x00000001 ;																	// Set pahse measure bit
		RamWrite32A( StMeasFunc_PMC_UcPhaseMesMode, UlReadVal ) ;									// Set Phase Measure Mode phase measure mode
		...
		RamRead32A( StMeasFunc_PMC_UcPhaseMesMode, &UlReadVal ) ;
		UlReadVal	&= 0xFFFFFDFF ;																	// reset wait timer bit
		RamWrite32A( StMeasFunc_PMC_UcPhaseMesMode, UlReadVal ) ;									// Set SweepMode enable wait timer
